...
namespace DFM_BPM.Forms
{
    public partial class PetWorkflow : Page
    {
        ...
        protected void btnAddLine_Click(object sender, EventArgs e)
        {
            ...
            hfEditLineId.Value = "";
            litLineModalTitle.Text = "New Line Item";
            ddlExpHead.SelectedIndex = 0;
            txtLineTopic.Text = txtLineDesc.Text = "";
            txtLineUnits.Text = "1"; txtLineUnitPrice.Text = txtLineAmtFCY.Text = txtLineAmtLCY.Text = "0";
            ...
            ScriptManager.RegisterStartupScript(this, GetType(), "showLineModal",
                "$(function(){ $('#lineModal').modal('show'); });", true);
        }

        protected void btnCancelLine_Click(object sender, EventArgs e)
        {
            // no-op: cancel is now handled by Bootstrap modal dismiss
        }
        ...
        protected void btnSaveLine_Click(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(hfEditLineId.Value, out editId) && editId > 0)
            {
                ...
                WorkflowDAL.UpdatePetLine(
                    editId, "", ddlExpHead.SelectedValue,
        ...
        protected void gvLines_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "EditLine" && IsEditable)
            {
                int lineId = Convert.ToInt32(e.CommandArgument);
                DataRow r  = Db.QueryRow(
                    "SELECT * FROM dbo.PetLineItem WHERE LineID=@id", Db.P("@id", lineId));
                if (r == null) return;

                hfEditLineId.Value = lineId.ToString();
                litLineModalTitle.Text = "Edit Line #" + (r["SerialNo"] == DBNull.Value ? lineId.ToString() : r["SerialNo"].ToString());
